...
def generate_data():
    ...
    t_l, u_l, t_r, u_r = t[lb:(le+1), :], u[lb:(le+1), :], t[rb:re, :], u[rb:re, :]
    ...
    #idx_u_r = np.random.choice(t.shape[0] - le, num_u_train, replace=False) + le
...
if __name__ == '__main__':
    ...
    t_l_bc, t_r_bc = torch.tensor(t_l_bc, dtype=torch.float32), torch.tensor(t_r_bc, dtype=torch.float32)
    ...
    while n < nmax:
        ...
        loss_coupling = torch.mean((u_l_bc_pred - u_l_bc_ref)**2) + torch.mean((u_r_bc_pred - u_r_bc_ref)**2)
    ...
    plt.figure()
    plt.plot(t_u_l_train.numpy(), u_l_train.numpy(), 'mo')
    plt.plot(t_u_r_train.numpy(), u_r_train.numpy(), 'mX')
    plt.plot(t_ref, u_ref, 'k-')
    plt.plot(t_l_ref, u_l_test.detach().numpy(), 'r--')
    plt.plot(t_r_ref, u_r_test.detach().numpy(), 'b--')
    plt.show()
