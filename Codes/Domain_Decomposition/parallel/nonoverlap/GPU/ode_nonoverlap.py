...
def main(rank, world_size, train_dataset_0, train_dataset_1, test_dataset_0, test_dataset_1, test_dataset):
    ...
    if rank == 0:
        t_u_train, u_train = train_dataset_0[0].to(rank), train_dataset_0[1].to(rank)
        t_f_train, f_train = train_dataset_0[2].to(rank), train_dataset_0[3].to(rank)
        t_coupling_bc = train_dataset_0[-1].to(rank)
        t_sub_ref, u_sub_ref = test_dataset_0[0], test_dataset_0[1]
        t_ref, u_ref = test_dataset[0], test_dataset[1]
    else:
        t_u_train, u_train = train_dataset_1[0].to(rank), train_dataset_1[1].to(rank)
        t_f_train, f_train = train_dataset_1[2].to(rank), train_dataset_1[3].to(rank)
        t_coupling_bc = train_dataset_1[-1].to(rank)
    ...
    a = torch.tensor(1.5, dtype=torch.float32).to(rank)
    ...
    while n < nmax:
        ...
        if rank == 0:
            dist.isend(u_coupling_bc_pred, dst=(rank+1)%world_size).wait()
    ...
    u_test = model(torch.tensor(t_sub_ref, dtype=torch.float32).to(rank))
