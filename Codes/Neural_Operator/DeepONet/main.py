...
def main():
    ...
    u_f = model.fnn_B(f_ph, W_g_f, b_g_f) #[batch_size, G_dim]
    u_x = model.fnn_T(x, W_g_x, b_g_x, Xmin, Xmax) #[x_num, G_dim]
    u_pred = tf.einsum('bi, ji->bj', u_f, u_x)
    ...
    test_dict = {f_ph: f_test, u_ph: u_test}
    u_pred_test_ = sess.run(u_pred, feed_dict=test_dict)
