...
def main(rank, world_size, train_dataset, test_dataset):
    ...
    train_inputs = x_train[(rank*bs):(rank+1)*bs, :].to(device)
    train_targets = y_train[(rank*bs):(rank+1)*bs, :].to(device)
    ...
    save_dict = {'x_train': train_inputs.cpu().numpy(), 'y_train': train_targets.cpu().numpy(), \
                 'x_test': x_ref, 'y_pred': y_test.cpu().detach().numpy(), 'y_ref': y_ref}
